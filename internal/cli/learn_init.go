...
func initLearn(ctx context.Context, db *sql.DB) error {
	...
	seeds := map[string][]lookups.SeedConfig{
		"disclosure_template": {
			{Canonical: "Quarterly Report", Values: []string{"17-q", "quarterly"}},
			{Canonical: "Annual Report", Values: []string{"17-a", "annual"}},
			{Canonical: "Material Information/Transactions", Values: []string{"material information", "matinfo"}},
			{Canonical: "Clarification of News Reports", Values: []string{"clarification"}},
			{Canonical: "Declaration of Cash Dividends", Values: []string{"cash dividend", "dividends"}},
		},
		"index": {
			{Canonical: "PSEi", Values: []string{"psei index", "composite index", "pse composite"}},
			{Canonical: "All Shares", Values: []string{"all shares index"}},
		},
	}
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: pse-edge-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: pse-edge-pp-cli: learn init: %v\n", err)
		}
	})
}
