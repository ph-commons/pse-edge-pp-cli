...
var whichIndex = []whichEntry{
	{Command: "history", Description: "Query daily OHLC/value history for any ticker or the PSEi from the local store — data no free PSE API serves.", Group: "Local history that compounds", WhyItMatters: "Reach for this when a question spans more than the current session; every row carries source and as-of trading date."},
	{Command: "drift", Description: "Percent change over a window, absolute and versus the PSEi over the same sessions, with 52-week band position.", Group: "Local history that compounds", WhyItMatters: "Answers 'how has this done vs the market' in one call instead of two fetches and hand math."},
	{Command: "session", Description: "Philippine trading-calendar verdict: trading day or not, pre/post-close gate, last completed trading date.", Group: "As-of discipline", WhyItMatters: "Gate every EOD figure on this before trusting it — blank change fields on non-trading days are states, not zeros."},
	{Command: "stale", Description: "Per-ticker last-synced trading date versus the last completed trading day, with in-series gaps listed.", Group: "As-of discipline", WhyItMatters: "Run before analytics so a silently lagged sync cannot masquerade as a market move."},
	{Command: "deadlines", Description: "Computed 17-Q/17-A due dates per SRC Rule 68 45/105-day windows, joined to local disclosures: filed, pending, or overdue.", Group: "Filing rhythm", WhyItMatters: "Answers 'has the quarterly report landed and when is it due' without a browser search."},
	{Command: "filings", Description: "Search PSE Edge disclosures by ticker (or --all); upserts headers into local pse_disclosures. Not an authoritative complete corpus — read warnings/complete/freshness_gap_days.", Group: "Filing rhythm", WhyItMatters: "Pull a company's disclosure list and feed the deadlines index; never treat a successful response as proof every filing exists."},
	{Command: "filings get", Description: "Direct openDiscViewer.do lookup by edge_no — structured company/title/date/attachments when search omits a known filing.", Group: "Filing rhythm", WhyItMatters: "Authoritative path for a known edge_no missing from announcements/search.ax (issue #10). Prefer over disclosures view for agent JSON."},
	{Command: "filings day", Description: "Download one Manila publication day's disclosure bodies and attachments into a local evidence bundle. Search discovery is not a complete corpus.", Group: "Filing rhythm", WhyItMatters: "Retains original bytes, hashes, and a manifest for one calendar day without treating announcements/search.ax as exhaustive."},
	{Command: "export", Description: "JSONL/JSON export: live companies, or local-store eod/index/companies-local with versioned contract ids (pse-edge-export-*-v1).", Group: "Downstream boundary", WhyItMatters: "Stable handoff for private analytics pipelines without treating data.db as a public ABI (issue #9)."},
	{Command: "breadth", Description: "Advance/decline/unchanged and value-traded trend over time from local index snapshots.", Group: "Local history that compounds", WhyItMatters: "Breadth direction over weeks is the tape context a single day's number cannot give."},
	{Command: "movers", Description: "Ranked gainers and losers across the synced universe for a window, with universe size and as-of stated.", Group: "Local history that compounds", WhyItMatters: "Weekend tape review in one call; the output names its universe so partial syncs cannot mislead."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  pse-edge-pp-cli which "stale tickets"
  pse-edge-pp-cli which "bottleneck"
  pse-edge-pp-cli which --limit 1 "send message"
  pse-edge-pp-cli which                                # list the full capability index`,
