...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PSE_EDGE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PSE_EDGE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/ngpestelos/pse-edge-pp-cli/feedback")
...
When ` + "`PSE_EDGE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PSE_EDGE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pse-edge-pp-cli feedback list
  pse-edge-pp-cli feedback list --limit 5
  pse-edge-pp-cli feedback list --json`,
