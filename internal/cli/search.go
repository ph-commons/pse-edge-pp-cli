...
In local mode: searches locally synced data only.`,
  ...
  pse-edge-pp-cli search "error timeout"

  # Force local search only
  pse-edge-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  pse-edge-pp-cli search "status" --type companies --data-source local
  # JSON output for piping
  pse-edge-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /announcements/search.ax
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/announcements/search.ax", map[string]any{
					"keyword":   query,
					"companyId": "",
					"tmplNm":    "",
					"fromDate":  "",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pse-edge-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
