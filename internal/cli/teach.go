...
// pse-edge-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
const noLearnEnvVar = "PSE_EDGE_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  pse-edge-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  pse-edge-pp-cli recall "<question>" --agent
  pse-edge-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pse-edge-pp-cli learnings list --agent
  pse-edge-pp-cli learnings list --query "<substring>"
  pse-edge-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  pse-edge-pp-cli learnings forget "<question>" --resource <id>
  pse-edge-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  pse-edge-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  pse-edge-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
