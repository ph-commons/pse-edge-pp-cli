...
Resource scoping:
  ...
  pse-edge-pp-cli sync

  # Sync specific resources only
  pse-edge-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pse-edge-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  pse-edge-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pse-edge-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pse-edge-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "pse-edge-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "companies":
		...
		return db.UpsertCompanies(typedData)
	case "prices":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertPrices(typedData)
	case "quotes":
		typedData, err := typedSingleObjectData(data, obj, id, forceTypedID)
		if err != nil {
			return err
		}
		return db.UpsertQuotes(typedData)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"companies": "/autoComplete/searchCompanyNameSymbol.ax",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "prices\x00/common/DisclosureCht.ax":
